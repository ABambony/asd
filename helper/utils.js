...
const requireSessionConditions = (
    ...
    if (!channel) {
        interactionOrMessage.reply?.({
            embeds: [
                new EmbedBuilder()
                    .setColor(errorColor)
                    .setDescription(`${joinvc} Пожалуйста, присоединитесь к голосовому каналу.`)
            ],
            ephemeral: true
        });
        return false;
    ...
    if (queue && queue.channel.id !== channel.id) {
        interactionOrMessage.reply?.({
            embeds: [
                errorEmbed(`Я играю в <#${queue.channel.id}>`)
            ],
            ephemeral: true
        });
        return false;
    ...
    if (
        ...
        interactionOrMessage.reply?.({
            embeds: [
                errorEmbed(`В данный момент песня не воспроизводится - используйте \`/play\` чтобы запустить проигрывание.`)
            ],
            ephemeral: true
        });
        return false;
...
const requireInitializeSessionConditionsUnified = (interactionOrMessage) => {
    ...
    if (!channel.viewable) {
        interactionOrMessage.reply?.({
            embeds: [
                errorEmbed(`У меня нет прав чтобы подключаться к этому голосовому каналу.`)
            ],
            ephemeral: true
        });
        return false;
    ...
    if (
        ...
        interactionOrMessage.reply?.({
            embeds: [
                errorEmbed(`Этот голосовой канал заполнен.`)
            ],
            ephemeral: true
        });
        return false;
    ...
    for (let i = 0; i < currQueue.length; i += chunkSize) {
        ...
        embed
            .setDescription(stripIndents`
              ${disk} ** | Сейчас играет: ** [${escapeMarkdown(queue.currentTrack.title)}](${queue.currentTrack.url})${typeof queue.repeatMode !== 'undefined' && queue.repeatMode !== null ? `\n${mode} ** | Режим повторения: ** ${repeatModeStr}` : ''}
    
              ${chunkOutput}
            `)
            .setFooter({
                text: `Страница ${Math.ceil((i + chunkSize) / chunkSize)} от ${Math.ceil(currQueue.length / chunkSize)
                    } [${i + 1}-${Math.min(i + chunkSize, currQueue.length)} / ${currQueue.length}]${queue.estimatedDuration ? `\nДлительность: ${msToHumanReadableTime(queue.estimatedDuration)}` : ''}`
            });
...
const queueEmbedResponse = (messageOrInteraction, queue, title = 'Очередь') => {
...
const handlePaginationButtons = (i, componentMember, pageNow, prevCustomId, nextCustomId, usableEmbeds) => {
    ...
    // Prev Button - Check bounds
        ...
        i.reply({
            embeds: [
                errorEmbed(`Вы уже на первой странице.`)
            ],
            ephemeral: true
        });
        return false;
    ...
    // Next - Check bounds
        ...
        i.reply({
            embeds: [
                errorEmbed(`Вы уже на последней странице.`)
            ],
            ephemeral: true
        });
        return false;
...
const msToHumanReadableTime = (ms) => {
    ...
    const parts = [];
    if (days > 0) parts.push(`${days} дней${days === 1 ? '' : 's'}`);
    if (hours > 0) parts.push(`${hours} часов${hours === 1 ? '' : 's'}`);
    if (minutes > 0) parts.push(`${minutes} минут${minutes === 1 ? '' : 's'}`);
    if (seconds > 0) parts.push(`${seconds} секунд${seconds === 1 ? '' : 's'}`);

    if (parts.length === 0) return '0 секунд';
...
const secondsToHumanReadableTime = (seconds) => {
    ...
    if (hours > 0) parts.push(`${hours} часов${hours === 1 ? '' : 's'}`);
    if (minutes > 0) parts.push(`${minutes} минут${minutes === 1 ? '' : 's'}`);
    if (secs > 0) parts.push(`${secs} секунд${secs === 1 ? '' : 's'}`);
...
const nowPlayingEmbed = (interactionOrMessage, client, queue) => {
    ...
    const np = new EmbedBuilder()
        .setColor(botColor)
        .setAuthor({
            name: `Информация о текущей песне ${bottomArrow}`,
        ...
        .addFields(
            { name: `${leftAngleDown} Длительность`, value: `${arrow} ${currentTrack.duration === '0:00' ? 'Прямой эфир' : currentTrack.duration}`, inline: true },
            { name: `${leftAngleDown} Артист`, value: `${arrow} ${currentTrack.author}`, inline: true },
            { name: `${leftAngleDown} Источник`, value: `${arrow} ${titleCase(currentTrack.source)}`, inline: true },
        )
        .addFields(
            { name: `${leftAngleDown} Режим повторения`, value: `${arrow} ${repeatModeEmojiStr(queue.repeatMode)}`, inline: true },
            { name: `${leftAngleDown} Громкость`, value: `${arrow} ${queue.options.volume} %`, inline: true },
            { name: `${leftAngleDown} Ссылка на песню`, value: `${arrow} [Нажмите сюда](${currentTrack.url})`, inline: true },
        )
        .setFooter({
            iconURL: interactionOrMessage?.user?.displayAvatarURL() || interactionOrMessage?.author?.displayAvatarURL() || "NA",
            text: `Запрошено ${interactionOrMessage?.user?.username || interactionOrMessage?.author?.username || "NA"}`
        })
        .setTimestamp()
...
const saveSongEmbed = (interactionOrMessage, client, queue) => {
    ...
    const np = new EmbedBuilder()
        .setColor(botColor)
        .setAuthor({
            name: `Сохранённые песни ${bottomArrow}`,
            iconURL: client.user.displayAvatarURL()
        })
        .setTitle(`**${currentTrack.source === 'youtube' ? currentTrack.cleanTitle || currentTrack.title : currentTrack.title}**`)
        .setURL(currentTrack.url)
        ...
        .addFields(
            { name: `${leftAngleDown} Загружено`, value: `${arrow} ${currentTrack.metadata?.uploadedAt || currentTrack.metadata?.source?.releaseDate.isoString ? new Date(currentTrack.metadata?.source?.releaseDate.isoString).toLocaleDateString('ru-RU') : 'NA'}`, inline: true },
            { name: `${leftAngleDown} Понравилось`, value: `${arrow} ${currentTrack.raw?.likes || currentTrack.metadata.bridge?.likes || 'NA'}`, inline: true },
            { name: `${leftAngleDown} Ссылка на песню`, value: `${arrow} [Нажмите сюда](${currentTrack.url})`, inline: true },
        )
        .setFooter({
            iconURL: interactionOrMessage?.user?.displayAvatarURL() || interactionOrMessage?.author?.displayAvatarURL() || "NA",
...
const startedPlayingMenu = async (queue, track) => {
    ...
    const selectMenu = new StringSelectMenuBuilder()
        ...
        .setPlaceholder('Добавьте предложенную песню в очередь')
        .addOptions(options);
...
const startedPlayingEmbed = (queue, track, client) => {
    ...
    return new EmbedBuilder()
        ...
        .setAuthor({
            ...
            name: `Сейчас играет ${bottomArrow}`
        })
        .setTitle(`${disk} ${track.title}`)
        .setURL(track.url)
        // .setThumbnail(track.thumbnail)
        .addFields(
            { name: `${leftAngleDown} Длительность`, value: `${arrow} ${track.duration === '0:00' ? 'Прямой эфир' : track.duration}`, inline: true },
            { name: `${leftAngleDown} Артист`, value: `${arrow} ${track.author}`, inline: true },
            { name: `${leftAngleDown} Режим повторения`, value: `${arrow} ${repeatModeEmojiStr(queue.repeatMode)}`, inline: true },
            { name: `${leftAngleDown} Следующая песня`, value: `${arrow} ${nextTrack ? `[${nextTrack.cleanTitle}](${nextTrack.url})` : 'Очередь пуста.'}`, inline: false }
        )
        // .addFields(
        ...
        .setFooter({
            iconURL: client.user.displayAvatarURL(),
            text: `Запрошено ${track.requestedBy.username}.`
        })
