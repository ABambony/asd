...
module.exports = {
	...
	data: new SlashCommandBuilder()
		.setName('remove')
		.setDescription("Убрать песню из очереди.")
	...
	async execute(interaction, client) {
		...
		try {
			...
			if ((queue?.size ?? 0) < 2) {
				await interaction.reply({ embeds: [errorEmbed(` В очереди недостаточно песен для удаления.`)] });
			...
			if (songPosition > queueSizeZeroOffset) {
				await interaction.reply({
					embeds: [errorEmbed(` The \`${SONG_POSITION_OPTION_ID + '` Заданная позиция'
						} не находится в диапазоне 1-${queue.size}`)]
				});
				setTimeout(() => interaction.deleteReply(), ERROR_MSGE_DELETE_TIMEOUT)
				return;
			}
			...
			if (songPosition === 0) {
				if (queue.metadata?.nowPlaying) {
					...
					if (fieldIndex !== -1) {
						embedObject.fields[fieldIndex].value = `${arrow} ${nextTrack ? `[${nextTrack.cleanTitle}](${nextTrack.url})` : 'Очередь пуста.'}`
					} else {
						await interaction.reply({ embeds: [errorEmbed(`Произошла ошибка при обновлении текущей песни.`)] })
						setTimeout(() => {
			...
			await interaction.reply({ embeds: [successEmbed(` [${escapeMarkdown(track.title)}](${track.url}) убран из очереди - By ${interaction.user}`)] });
			setTimeout(() => interaction.deleteReply(), BOT_MSGE_DELETE_TIMEOUT)

		} catch (error) {
			await interaction.reply({
				embeds: [
					errorEmbed(`Что-то пошло не так, при использовании этой: \`/remove\` команды.`)
				],
				ephemeral: true
			});
			errorLog(error)
