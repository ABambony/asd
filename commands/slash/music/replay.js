    ...
    async execute(interaction, client) {
        ...
        try {            
            await interaction.reply({
                embeds: [
                    new EmbedBuilder()
                    .setDescription(` Повторяем, пожалуйста ожидайте... ${wait}`)
                ]
            });
            ...
            await interaction.editReply({ embeds: [successEmbed(` Повторение текущей песни - By ${interaction.user}`)] });
            ...
            if (queue.metadata?.nowPlaying) {
                ...
                try {
                    ...
                    const progressBarFieldIndex = embed.fields.findIndex(field => field.name === `${cyanDot} Процесс ${bottomArrow}`);
            ...
            if(interaction.replied || interaction.deferred){
                await interaction.editReply({
                    embeds: [
                        errorEmbed(`Что-то пошло не так, при использовании этой: \`/replay\` команды.`)
                    ],
                    ephemeral: true
                });
            }else{
