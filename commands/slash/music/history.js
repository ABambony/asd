...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('history')
        .setDescription('Показать текущую историю.'),
    ...
    async execute(interaction, client) {
        ...
        try {
            ...
            if (!history) {
                await interaction.reply({ embeds: [errorEmbed(`В истории ничего нет.`)] })
            ...
            await interaction.reply({
                embeds: [
                    errorEmbed(`Что-то пошло не так, при использовании этой: \`/history\` команды.`)
                ],
                ephemeral: true
            });
            errorLog(error)
